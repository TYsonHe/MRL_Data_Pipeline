...
def resizeImage(imgFile, inputW, inputH, targetImgW, targetImgH, outputImgFile):
...
if __name__ == "__main__":
    ...
    for img in data['images']:
        print("Processing image file {0} and its captions...".format(
            img['file_name']))
        ...
        outNewImg = resizeImage(imgFullPath, int(
            img['width']), int(img['height']), targetImgW, targetImgH, outputImgFullPath)

        for i in range(0, len(annList)):
            annId = annList[i]['id']

            # 更改caption
            data['annotations'][annsIdx[annId]
                                ]['caption'] = preprocess_text(annList[i]['caption'])
